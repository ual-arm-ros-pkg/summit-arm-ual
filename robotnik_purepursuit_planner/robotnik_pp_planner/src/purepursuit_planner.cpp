...
#define ODOM_TIMEOUT_ERROR \
    0.2  // max num. of seconds without receiving odom values
#define MAP_TIMEOUT_ERROR \
    0.2  // max num. of seconds without receiving map transformations
#define AGVS_TURN_RADIUS \
    0.20  // distancia en la que empieza a girar el robot cuando llega a una
          // esquina
#define MIN_ANGLE_BEZIER \
    0.261799388  // ángulo (radianes) mínimo entre segmentos de la recta para
                 // los que ajustaremos a una curva de BEZIER
#define BEZIER_CONTROL_POINTS 5

#define D_LOOKAHEAD_MIN \
    0.3  // Minima distancia del punto objetivo en m (PurePursuit con lookahead
         // dinámico)
#define D_LOOKAHEAD_MAX 1.1  // Maxima distancia del punto objetivo
#define D_WHEEL_ROBOT_CENTER \
    0.478  // Distance from the motor wheel to the robot center

#define MAX_SPEED_LVL1 0.5
#define MAX_SPEED_LVL2 0.3
#define MAX_SPEED 1.2

#define WAYPOINT_POP_DISTANCE_M \
    0.10  // Distancia mínima para alcanzar punto objetivo m (PurePursuit)

#define AGVS_FIRST_DECELERATION_DISTANCE \
    0.5  // meters -> when the vehicle is arriving to the goal, it has to
         // decelarate at this distance
#define AGVS_FIRST_DECELERATION_MAXSPEED 0.15  // m/s
#define AGVS_SECOND_DECELERATION_DISTANCE \
    0.25  // meters -> when the vehicle is arriving to the goal, it has to
          // decelarate another time at this distance
...
class Path
{
   ...
   public:
    ...
    Path()
    {
        ...
        pthread_mutex_init(
            &mutexPath, NULL);  // Initialization for WaypointRoutes' mutex
        bOptimized = false;
    }

    //! Destructor
    ~Path() { pthread_mutex_destroy(&mutexPath); }
    ...
    ReturnValue AddWaypoint(vector<Waypoint> po)
    {
        ...
        for (int i = 0; i < po.size(); i++) { vPoints.push_back(po[i]); }
        pthread_mutex_unlock(&mutexPath);
    }
    ...
    ReturnValue AddMagnet(vector<MagnetStruct> po)
    {
        ...
        for (int i = 0; i < po.size(); i++) { vMagnets.push_back(po[i]); }
        pthread_mutex_unlock(&mutexPath);
    }
    ...
    unsigned int Size() { return vPoints.size(); }
    ...
    ReturnValue GetNextWaypoint(Waypoint* wp)
    {
        ...
        if ((iCurrentWaypoint >= 0) &&
            (iCurrentWaypoint < (vPoints.size() - 1)))
        {
    ...
    int GetCurrentWaypointIndex() { return iCurrentWaypoint; }
    ...
    int NumOfWaypoints() { return vPoints.size(); }
    ...
    int GetCurrentMagnetIndex() { return iCurrentMagnet; }
    ...
    int NumOfMagnets() { return vMagnets.size(); }
    ...
    //! Cross product
    double dot2(Waypoint w, Waypoint v) { return (w.dX * v.dX + w.dY * v.dY); }
    ...
    Waypoint PosOnQuadraticBezier(
        Waypoint cp0, Waypoint cp1, Waypoint cp2, float t)
    {
        Waypoint aux;

        // B(t)= (1-t)^2 * cp0 + 2*t*(1-t)*cp1 + t^2 * cp2;
        // Bx(t)
        aux.dX = (1.0 - t) * (1.0 - t) * cp0.dX + 2 * t * (1.0 - t) * cp1.dX +
                 t * t * cp2.dX;
        // By(t)
        aux.dY = (1.0 - t) * (1.0 - t) * cp0.dY + 2 * t * (1.0 - t) * cp1.dY +
                 t * t * cp2.dY;
    ...
    //! \return ERROR if Size is lower than 3, distance <= 0 or the waypoints
    //! has already been optimized \return OK
    ReturnValue Optimize(double distance)
    {
        ...
        double   dMinDist  = 0.0;  // Minica distancia a la que hay q frenar en
        ...
        {  // Minimo 3 puntos en la ruta
            // printf("WaypointRoute::Optimize: Error: not enought points
            // (%d)\n",Size());
            return ERROR;
        }
        pthread_mutex_lock(&mutexPath);
        ...
        if (vPoints.size() == 2)
        {
            aux = vPoints[1];
            vPoints.push_back(
                aux);  // Añadimos un punto al final y modificamos el del medio
            if ((vPoints[0].dX - aux.dX) == 0.0)
            {  // Punto en el mismo eje X
                vPoints[1].dX = vPoints[0].dX;  // Mantenemos la coordenada en X
                vPoints[1].dY =
                    vPoints[0].dY +
                    (aux.dY - vPoints[0].dY) /
                        2.0;  // La coordenada en Y será igual a la del primer
                              // punto más la mitada de distancia entre el punto
                              // final e inicial
            }
            else if ((vPoints[0].dY - aux.dY) == 0.0)
            {  // Punto en el mismo eje Y
                vPoints[1].dX =
                    vPoints[0].dX +
                    (aux.dX - vPoints[0].dX) /
                        2.0;  // La coordenada en X será igual a la del primer
                              // punto más la mitada de distancia entre el punto
                              // final e inicial
                vPoints[1].dY = vPoints[0].dY;  // Mantenemos la coordenada en Y
            }
            else
            {  // Punto en eje X, Y distinto
                vPoints[1].dX =
                    vPoints[0].dX +
                    (aux.dX - vPoints[0].dX) /
                        2.0;  // La coordenada en X será igual a la del primer
        ...
        {  // Primera pasada, añadimos puntos para los giros en curvas
            ...
            {  // en caso del angulo que forman los segmentos sea menor,
               // entonces generaremos puntos para aproximar mjor la curva
                // siendo ba vector director de b->a y bc el vector director
                // b->c Calculamos un punto a una distancia 'd' desde 'b' hacia
                // 'a' y otro punto desde 'b' hacia 'c' Estos puntos serán los
                // que se añadirán a la lista de waypoints para poder trazar una
                // curva de bezier

                new_points.pop_back();

                // Calcula velocidad maxima en funcion del giro de la curva
                if (fabs(dAngle) >= (Pi / 4)) { dAuxSpeed = MAX_SPEED_LVL2; }
                else
                    dAuxSpeed = MAX_SPEED_LVL1;
                // cout << "Aux speed = " << dAuxSpeed << ", Next speed =  " <<
                // vPoints[b].dSpeed << endl;
                // Si la velocidad en ese waypoint supera el máximo establecido
                // para un giro así
                if (fabs(vPoints[b].dSpeed) > dAuxSpeed)
                {
                    ...
                    // Si el punto antes del giro esta a una distancia menor a
                    // la mínima, añadimos nuevo punto a un metro
                    if (mod_ab > dMinDist)
                    {
                        // Lo creamos
                        ba.dX = -ab.dX;
                        ...
                        aux.dY = By + K * ba.dY;  // y = y' + K*Vy	//(Vx, Vy)
                                                  // vector director
                        aux.dSpeed = vPoints[b].dSpeed;
                        // cout << "Nuevo punto en " << aux.dX << ", " << aux.dY
                        // << endl;
                ...
                if (mod_ab > distance)
                {  // si la distancia entre ab es MAYOR a la distancia del punto
                   // que pretendemos crear, creamos un punto intermedio
                    // Lo creamos
                    ba.dX = -ab.dX;
                    ba.dY = -ab.dY;
                ...
                if (mod_bc > distance)
                {  // si la distancia entre ab es menor a la distancia del punto
                   // que pretendemos crear, lo dejamos como está
                    // Lo creamos
                    K      = distance / sqrt(bc.dX * bc.dX + bc.dY * bc.dY);
                    aux.dX = Bx + K * bc.dX;  // x = x' + K*Vx
                    aux.dY = By + K * bc.dY;  // y = y' + K*Vy	//(Vx, Vy)
                                              // vector director
                    aux.dSpeed = vPoints[b].dSpeed;
                    // j++;
                    // cout << "Nuevo punto en " << aux.dX << ", " << aux.dY <<
                ...
                {
                    if (mod_bc > 1.0)
                    {  // si la distancia al punto C es mayor que 1 metro,
                       // después del giro
        ...
        {  // Segunda pasada, aproximamos los giros a curvas de Bezier
            a = i - 2;
            b = i - 1;
            c = i;
            ...
            if (fabs(dAngle) >= MIN_ANGLE_BEZIER)
            {  // en caso del angulo que forman los segmentos sea menor,
               // entonces generaremos puntos, siguiendo una curva de Bezier,
               // para aproximar mjor la curva
                // siendo ba vector director de b->a y bc el vector director
                // b->c
                ...
                aux_speed =
                    new_points[b]
                        .dSpeed;  // takes speed of the waypoint in the middle
                vPoints.pop_back();  // Eliminamos el waypoint del medio. El
                                     // primer waypoint no se modifica
                ...
                for (int j = 1; j <= BEZIER_CONTROL_POINTS; j++)
                {
                    ...
                    //	std::cout << "\tWaypointRoute::Optimize: (Bezier)
                    //Waypoint,X= " << aux_wp.dX << " Y= " << aux_wp.dY
...
class purepursuit_planner_node : public Component
{
   private:
    ...
    //! Saves the time whenever receives a command msg and a transform between
    //! map and base link (if configured)
    ros::Time last_command_time, last_map_time;
    // ACTIONLIB
    actionlib::SimpleActionServer<robotnik_pp_msgs::GoToAction>
                                   action_server_goto;
   ...
   public:
    ...
    void ROSSetup()
    {
        ...
        private_node_handle_.param(
            "cmd_topic_vel", cmd_topic_vel_, std::string("/cmd_vel"));
        private_node_handle_.param(
            "d_lookahear_min", d_lookahear_min_, D_LOOKAHEAD_MIN);
        private_node_handle_.param(
            "d_lookahear_max", d_lookahear_max_, D_LOOKAHEAD_MAX);
        private_node_handle_.param(
            "d_dist_wheel_to_center", d_dist_wheel_to_center_,
        ...
        private_node_handle_.param<std::string>(
            "position_source", position_source_, "ODOM");
        private_node_handle_.param(
            "desired_freq", desired_freq_, desired_freq_);
        private_node_handle_.param<std::string>(
            "command_type", s_command_type,
            COMMAND_TWIST_STRING /*COMMAND_ACKERMANN_STRING*/);
        private_node_handle_.param<std::string>(
            "target_frame", target_frame_, "/base_footprint");

        // private_node_handle_.param<std::string>("name_sc_enable_frot_laser_",
        // name_sc_enable_front_laser_, "/s3000_laser_front/enable_disable");
        // private_node_handl_.param<std::string>("name_sc_enable_back_laser",
        // name_sc_enable_back_laser_, "/s3000_laser_back/enable_disable"	);
        ...
        {
            //
            // Publish through the node handle Ackerman type messages to the
            // command vel topic
            vel_pub_ = private_node_handle_
                           .advertise<ackermann_msgs::AckermannDriveStamped>(
                               cmd_topic_vel_, 1);
        }
        else
        {
            ...
            // command vel topic
            vel_pub_ = private_node_handle_.advertise<geometry_msgs::Twist>(
                cmd_topic_vel_, 1);
        }
        ...
        if (ui_position_source == MAP_SOURCE)
            tranform_map_pub_ =
                private_node_handle_.advertise<geometry_msgs::TransformStamped>(
                    "map_location", 100);
        // status_pub_ =
        // private_node_handle_.advertise<purepursuit_planner::ControllerStatus>("status",
        // 1);
        odom_sub_ = private_node_handle_.subscribe<nav_msgs::Odometry>(
            odom_topic_, 1, &purepursuit_planner_node::OdomCallback, this);
        // cmd_vel_sub_ =
        // private_node_handle_.subscribe<purepursuit_planner::AckermannDriveStamped>("command",
        // 1, &purepursuit_planner_node::CmdVelCallback, this );
        ...
        min_odom_freq           = 5.0;
        max_odom_freq           = 100.0;
        updater_diagnostic_odom = new diagnostic_updater::TopicDiagnostic(
            odom_topic_, updater_diagnostic,
            diagnostic_updater::FrequencyStatusParam(
                &min_odom_freq, &max_odom_freq, 0.1, 10),
            diagnostic_updater::TimeStampStatusParam(0.001, 0.1));

        // Action server
        action_server_goto.registerGoalCallback(
            boost::bind(&purepursuit_planner_node::GoalCB, this));
        action_server_goto.registerPreemptCallback(
            boost::bind(&purepursuit_planner_node::PreemptCB, this));

        // Services
        // sc_enable_front_laser_ =
        // private_node_handle_.serviceClient<s3000_laser::enable_disable>(name_sc_enable_front_laser_);
        // sc_enable_back_laser_ =
        // private_node_handle_.serviceClient<s3000_laser::enable_disable>(name_sc_enable_back_laser_);

        ROS_INFO(
            "%s::ROSSetup(): odom_topic = %s, command_topic_vel = %s, position "
            "source = %s, desired_hz=%.1lf, min_lookahead = %.1lf, "
            "max_lookahead = %.1lf, kr = %.2lf, command_type = %s",
            sComponentName.c_str(), odom_topic_.c_str(), cmd_topic_vel_.c_str(),
            position_source_.c_str(), desired_freq_, d_lookahear_min_,
            d_lookahear_max_, Kr, s_command_type.c_str());

        // ROS_INFO("%s::ROSSetup(): laser_topics: front -> %s, back -> %s",
        // sComponentName.c_str(), name_sc_enable_front_laser_.c_str(),
        // name_sc_enable_back_laser_.c_str());
    ...
    ReturnValue Start()
    {
        ...
        {
            ROS_INFO(
                "agvs_controller::Start: the component's thread is already "
                "running");
    ...
    void InitState()
    {
        ...
        {
            if (CheckOdomReceive() == 0) SwitchToState(STANDBY_STATE);
        }
        else
        {
            if (!bInitialized) Setup();
            if (!bRunning) Start();
        }
    }
    ...
    void StandbyState()
    {
        ...
        else
        {
            if (bEnabled && !bCancel)
            {
                ...
                {
                    ROS_INFO(
                        "%s::StandbyState: route available",
    ...
    void ReadyState()
    {
        ...
        {
            ROS_INFO(
                "%s::ReadyState: Motion is disabled", sComponentName.c_str());
            SetRobotSpeed(0.0, 0.0);
        ...
        {
            ROS_INFO(
                "%s::ReadyState: Cancel requested", sComponentName.c_str());
            SetRobotSpeed(0.0, 0.0);
            SwitchToState(STANDBY_STATE);
            return;
        }
        ...
        {
            ROS_ERROR(
                "%s::ReadyState: Error on PurePursuit", sComponentName.c_str());
            bCancel = true;  // Activates the flag to cancel the mision
            SetRobotSpeed(0.0, 0.0);
            goto_result.route_result = -1;
            goto_feedback.percent_complete =
                100.0;  // Set the percent to 100% to complete the action
     ...
     *   \brief Updates (little by little) the variable lookahead depending of
    ...
    void UpdateLookAhead()
    {
        ...
        if ((desired_lookahead - 0.001) > dLookAhead) { dLookAhead += inc; }
    ...
    /*! \fn double DistP2S( Odometry current_position, Waypoint s0, Waypoint s1,
     *Waypoint *Pb) \brief obtains distance between the current position and
     *segment s0->s1, and returns the point Return: the shortest distance from p
     *to s (utm points) and the point of the segment that gives the shortest
     *distance
     */
    double DistP2S(
        geometry_msgs::Pose2D current_position, Waypoint s0, Waypoint s1,
        Waypoint* Pb)
    {
        double vx, vy, wx, wy;
    ...
    /*! \fn ReturnValue PointDlh(geometry_msgs::Pose2D current_position,
     * geometry_msgs::Pose2D *wp	) \brief Returns a point in a distance
     * dlookahead on the path \return OK \return ERROR
     */
    ReturnValue PointDlh(
        geometry_msgs::Pose2D current_position, geometry_msgs::Pose2D* wp)
    {
        int                   i, j = 0, k;
        ...
        d_seg = new double[size];  // array con la distancia entre puntos
        ...
        {
            if ((pathCurrent.GetWaypoint(i, &s0) == OK) &&
                (pathCurrent.GetWaypoint(i + 1, &s1) == OK))
            {
                d_seg[i] = Dist(s0.dX, s0.dY, s1.dX, s1.dY);

                d = DistP2S(
                    current_position, s0, s1,
                    &Pb1);  // Pb1 closest point on segment
                ...
                // ROS_INFO("PointDlh. Distance to segment %d(%.2lf,
                // %2.lf)->%d(%.2lf, %2.lf) = %.3lf,point (%.3lf, %.3lf) (DMIN =
                // %.3lf)", i, s0.dX, s0.dY, i+1, s1.dX, s1.dY, d, Pb1.dX,
                // Pb1.dY, dmin);
            }
            else
            {
                ROS_ERROR(
                    "%s::PointDlh: Error Getting waypoints",
                    sComponentName.c_str());
                return ERROR;
            }
        }

        // ROS_INFO("PointDlh:: Current waypoint index %d, next
        // %d",pathCurrent.GetCurrentWaypointIndex(), j);
        ...
        if (pathCurrent.GetCurrentWaypointIndex() != j)
        {
            ...
            {
                ROS_ERROR(
                    "%s::PointDlh: Error setting current waypoint to %d",
                    sComponentName.c_str(), j);
                return ERROR;
            }
            else
            {  // OK
                ...
                {  // Penultimo waypoint
                    ...
                    pathCurrent.GetCurrentWaypoint(
                        &w_before_last);  // Penultimo waypoint
                    pathCurrent.BackWaypoint(&w_last);  // Ultimo waypoint
                    // Distancia maxima = distancia entre el punto actual y el
                    // penultimo punto + más la distancia entre los dos ultimos
                    // puntos + un valor constante
                    // dMaxDistance = Dist(w_before_last.dX, w_before_last.dY,
                    // odomWhenLastWaypoint.px, odomWhenLastWaypoint.py) +
                    // Dist(w_last.dX, w_last.dY, w_before_last.dX,
                    // w_before_last.dY) + 0.1; ROS_INFO("%s::PointDlh: Penultimo
                    // punto. Robot en (%.3lf, %.3lf, %.3lf). Distancia máxima a
                    // recorrer = %.3lf m ", sComponentName.c_str(),
                    // odomWhenLastWaypoint.px, odomWhenLastWaypoint.py,
                    // odomWhenLastWaypoint.pa, dMaxDistance);
                }
            }
        }
        ...
        {
            ROS_ERROR(
                "%s::PointDlh: Error getting next waypoint %d",
                sComponentName.c_str(), j);
            return ERROR;
        }
        ...
        t  = (d_seg[k] - d2) /
            d_seg[k];  // Pendiente avoid div/0. En teoria no puede producirse
                       // pq dos waypoints consecutivos serán diferentes

        // 4- Obtain point with t parameter
        if ((pathCurrent.GetWaypoint(k, &s0) == OK) &&
            (pathCurrent.GetWaypoint(k + 1, &s1) == OK))
        {
    ...
    int PurePursuit()
    {
        ...
        double curv, yaw;
        double wref;  //, epw, uw;
        // double d = D_WHEEL_ROBOT_CENTER;   // Length in m (equiv to curv
        // radius)
        ...
        {
            ROS_ERROR(
                "%s::PurePursuit: not enought waypoints",
                sComponentName.c_str());
            return -1;
        }
        ...
        {
            ROS_ERROR(
                "%s::PurePursuit: Error getting next point in the route",
                sComponentName.c_str());
            return -1;
        }
        //
        ...
        {
            ROS_ERROR(
                "%s::PurePursuit: Error getting the last point in the path",
                sComponentName.c_str());
            return -1;
        }

        double dAuxDist = Dist(
            current_position.x, current_position.y, last_waypoint.dX,
            last_waypoint.dY);  // dist(waypoints.back().pos, current_position);

        if (pathCurrent.GetNextWaypoint(&next_waypoint) == ERROR)
        {
            ROS_ERROR(
                "%s::PurePursuit: Error getting next waypoint in the path",
                sComponentName.c_str());
            return -1;
        }
        ...
        ROS_INFO(
            "Purepursuit: current pos (%.2lf, %.2lf), next pos (%.2lf, %.2lf), "
            "lookahead %.2lf, yaw = %.3lf, curv = %.3lf, dth = %.3lf, wref = "
            "%.3lf(%.3lf), speed=%.3lf",
            current_position.x, current_position.y, next_position.x,
            next_position.y, dLookAhead, yaw, curv, dth, wref, aux_wref,
            dAuxSpeed);
        // ROS_INFO("Purepursuit: yaw = %.3lf, curv = %.3lf, dth = %.3lf, wref =
        // %.3lf", yaw, curv, dth, wref);

        ////////////////// Sets the speed depending of distance or speed
        ///restrictions /////////
        ///////////////////////////////////////////////////////////////////////////////////////
        ...
        //
        // When the robot is on the last waypoint, checks the distance to the
        // end
        if (pathCurrent.GetCurrentWaypointIndex() >=
            (pathCurrent.NumOfWaypoints() - 2))
        {
            ret           = -10;
            double ddist2 = Dist(
                current_position.x, current_position.y, last_waypoint.dX,
                last_waypoint.dY);
            // Distancia recorrida
            // dDistCovered = Dist( current_position.px, current_position.py,
            // odomWhenLastWaypoint.px, odomWhenLastWaypoint.py);
            if (ddist2 < WAYPOINT_POP_DISTANCE_M)
            {
                ...
                ROS_INFO(
                    "%s::PurePursuit: target position reached (%lf, %lf, %lf). "
                    "Ending current path",
                    sComponentName.c_str(), current_position.x,
                    current_position.x, current_position.theta * 180.0 / Pi);
    ...
    void CancelPath()
    {
        ...
        ROS_INFO(
            "%s::CancelPath: action server preempted", sComponentName.c_str());
        ...
        if (ui_position_source == MAP_SOURCE)
        {
            try
            {
                listener.lookupTransform(
                    "/map", target_frame_, ros::Time(0), transform);
            ...
            {
                // ROS_ERROR("%s::AllState: %s", sComponentName.c_str(),
                // ex.what());
            }
        }
     ...
     * Checks whether or not it's receiving odom values and/or map
     * transformations \return 0 if OK \return -1 if ERROR
     */
    int CheckOdomReceive()
    {
        ...
        {
            if (ui_position_source == MAP_SOURCE and
                ((ros::Time::now() - last_map_time).toSec() >
                 MAP_TIMEOUT_ERROR))
                return -1;
            else
                return 0;
     ...
     *	\brief Calcula el sentido de movimiento de una ruta, dependiendo de la
     *posición inicial y el ángulo del robot \return 1 si el sentido es positivo
    ...
    int CalculateDirectionSpeed(Waypoint target_position)
    {
        ...
        { return 0; }
        // Cálculo del vector director del robot respecto al sistema de
        ...
        // Cálculo del ángulo entre el vector director y el vector al punto
        // objetivo
        beta = acos(
            (ux * vx + uy * vy) /
            (sqrt(ux * ux + uy * uy) * sqrt(vx * vx + vy * vy)));

        // Devolvemos valor dependiendo del ángulo entre la orientación del
        // robot y la posición objetivo (radianes) Tendremos en cuenta el valor
        // del sentido de avance de la última ruta.
        ...
        ROS_INFO(
            "%s:CalculateDirectionSpeed:  case %d. Beta = %.2lf. Diff = %.2lf. "
            "Last direction = %d, new direction = %d",
            sComponentName.c_str(), iCase, beta * 180.0 / M_PI,
            (beta - pi_medios) * 180.0 / M_PI, last_direction, ret);
    ...
    int CalculateDirectionSpeed(geometry_msgs::Pose2D target_position)
    {
        ...
        //
        // si la posicion objetivo es la misma del robot, devolvemos 0
        if ((target_position.x == x) && (target_position.y == y)) { return 0; }
        // Cálculo del vector director del robot respecto al sistema de
        // coordenadas del robot
        ux = cos(alpha);
        uy = sin(alpha);
    ...
    ReturnValue MergePath()
    {
        ...
        {
            goto_goal.target = action_server_goto.acceptNewGoal()
                                   ->target;  // Reads points from action server
            if (goto_goal.target.size() > 0)
            {
                if (goto_goal.target.size() > 1)
                {  // Tries to use the second point of the route
                    direction =
                        CalculateDirectionSpeed(goto_goal.target[1].pose);
                }
                else
                {
                    direction =
                        CalculateDirectionSpeed(goto_goal.target[0].pose);
                }

                if (direction == 1)
                {  // Uses only front laser
                    SetLaserFront();
                }
                else
                {  // Uses only back laser
                    SetLaserBack();
                }
                ...
                {
                    ROS_INFO(
                        "%s::MergePath: Point %d (%.3lf, %.3lf, %.3lf) speed = "
                        "%.3lf",
                        sComponentName.c_str(), i, goto_goal.target[i].pose.x,
                    ...
                    // Depending on the calculated motion direction, applies
                ...
                if (pathFilling.Optimize(AGVS_TURN_RADIUS) != OK)
                    ROS_ERROR(
                        "%s::GoalCB: Error optimizing the path",
                        sComponentName.c_str());
                ...
                goto_feedback.percent_complete =
                    0.0;  // Inits the feedback percentage
                ...
                if (qPath.size() > 0)
                {
                    ...
                    ROS_INFO(
                        "%s::MergePath: Adding new %d points from (%.2lf, "
                        "%.2lf) to (%.2lf, %.2lf) and %d magnets",
                        sComponentName.c_str(), aux.NumOfWaypoints(), wFirst.dX,
                        wFirst.dY, wLast.dX, wLast.dY, aux.NumOfMagnets());
                    ROS_INFO(
                        "%s::MergePath: Current number of points = %d and "
                        "magnets = %d",
                        sComponentName.c_str(), pathCurrent.NumOfWaypoints(),
                    ...
                    if (pathCurrent.NumOfWaypoints() < 2)
                    {
                        if (pathCurrent.CreateInterpolatedWaypoint(
                                this->pose2d_robot) == ERROR)
                        {
                            ROS_ERROR(
                                "%s::MergePath: Error adding an extra point",
                                sComponentName.c_str());
                        }
                    }

                    ROS_INFO(
                        "%s::MergePath: New number of points = %d and magnets "
                        "= %d",
                        sComponentName.c_str(), pathCurrent.NumOfWaypoints(),
                        pathCurrent.NumOfMagnets());
    ...
    bool SetLaserFront()
    {
        ...
        ROS_INFO("%s::SetLaserFront: Setting laser back to false, ret = %d",
        sComponentName.c_str(), srv.response.ret);

        srv.request.value = true;
        sc_enable_front_laser_.call(srv);
        ROS_INFO("%s::SetLaserFront: Setting laser front to true, ret = %d",
        sComponentName.c_str(), srv.response.ret);*/
    }
    ...
    bool SetLaserBack()
    {
        ...
        srv.request.value = false;
        sc_enable_front_laser_.call(srv);
        ROS_INFO("%s::SetLaserBack: Setting laser front to false, ret = %d",
        sComponentName.c_str(), srv.response.ret);

        srv.request.value = true;
        sc_enable_back_laser_.call(srv);
        ROS_INFO("%s::SetLaserBack: Setting laser back to true, ret = %d",
        sComponentName.c_str(), srv.response.ret);*/
    }
