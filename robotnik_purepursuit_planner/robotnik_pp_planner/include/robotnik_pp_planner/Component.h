 ...
 *Adaptado para ROS Define e implementa una serie de métodos y estructuras
 *básicas que seguirán todos los componentes desarrollados por Robotnik (C) 2013
 *Robotnik Automation, SLL All rights reserved.
 */
...
extern int clock_nanosleep(
    clockid_t __clock_id, int __flags, __const struct timespec* __req,
...
class Component
{
   ...
   protected:
    ...
    //! State before
    States iOldState;
    //!	Almacenaremos el nombre del componente para personalizar el log a de
   ...
   public:
    //! Creates new tasks
    ReturnValue CreateTask(
        int prio, double frec, void* (*start_routine)(void*));
};
