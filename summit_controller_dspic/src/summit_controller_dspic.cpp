...
summit_controller_dspic::summit_controller_dspic(std::string port)
{
    ...
    pthread_mutex_init(
        &mutex_odometry, NULL);  // Initialization for odometry's mutex
    ...
    dsPic_data.last_channel =
        DSPIC_FWD_DIRECTION;  // Default channel to be accessed

    // Create serial port
    serial = new SerialDevice(
        port.c_str(), DSPIC_DEFAULT_TRANSFERRATE, DSPIC_DEFAULT_PARITY,
        DSPIC_DEFAULT_DATA_SIZE);  // Creates serial device
    amMode             = GYRO;
...
void summit_controller_dspic::ReadyState()
{
    ...
    switch (iCmd)
    {
        case DSPIC_CMD_MODIFY_ODOMETRY:
            ret2 = SendModifyOdometry(
                dsPic_data_tmp.odometry_x, dsPic_data_tmp.odometry_y,
                dsPic_data_tmp.odometry_yaw);
            break;
        ...
        case DSPIC_CMD_CALIBRATE_OFFSET:
            ...
            SwitchToState(STANDBY_STATE);  // Stays on standby until checks the
                                           // new and right offset
            break;
        case DSPIC_CMD_SET_MOTOR_REFERENCES:
            ret2 =
                SendSetMotorReferences(dsPic_data_tmp.rad, dsPic_data_tmp.mps);
            break;
...
/*!	\fn int summit_controller_dspic::SendModifyOdometry(double x, double y,
 * double yaw) Changes odometry's current values \return OK \return ERROR
 */
...
/*!	\fn  int summit_controller_dspic::SendSetMotorReferences(double rad, double
 * mps) Sets the values for moving \return OK \return ERROR
 */
int summit_controller_dspic::SendSetMotorReferences(double rad, double mps)
{
    ...
    if (dsPic_data.bMotorsEnabled == true)
    {
        if (active_kinematic_mode_ == DUAL_ACKERMANN_INVERTED)
            sprintf(
                cMsg, "%s,%1.2f,%1.2f,%1.2f\r", SET_MOTOR_REFERENCES, rad, rad,
                mps);
        else if (active_kinematic_mode_ == SINGLE_ACKERMANN)
            sprintf(
                cMsg, "%s,%1.2f,%1.2f,%1.2f\r", SET_MOTOR_REFERENCES, rad, 0.0,
                mps);
        else if (active_kinematic_mode_ == DUAL_ACKERMANN_GEARED)
            sprintf(
                cMsg, "%s,%1.2f,%1.2f,%1.2f\r", SET_MOTOR_REFERENCES, rad, -rad,
                mps);
        else
...
/*!	\fn  int summit_controller_dspic::SendSetTrimmer(int channel, int offset1,
 * int center, int offset2) Sets the distance between wheels msg \return OK
 * \return ERROR
 */
...
/*!	\fn unsigned char summit_controller_dspic::ComputeCRC(char *string, int
 * size) Calculates the CRC for the message \return OK \return ERROR
 */
...
/*!	\fn int summit_controller_dspic::ModifyOdometry(double x, double y, double
 * yaw) Changes odometry's current values
 */
void summit_controller_dspic::ModifyOdometry(double x, double y, double yaw)
{
    if (bLocalOdometry)
    {
        ...
        dsPic_data.dOffsetGyro =
            (dsPic_data.dGyro -
             yaw);  // Changes the offset to the current value
        pthread_mutex_unlock(&mutex_odometry);
    }
    else
    {
...
void summit_controller_dspic::InitOdometry()
{
    ...
    {
        pthread_mutex_lock(&mutex_odometry);
        dsPic_data.odometry_x       = dsPic_data.odometry_y =
            dsPic_data.odometry_yaw = 0.0;
...
{
    if (bLocalOdometry) { dsPic_data.dDWheels = distance; }
    else
    {
...
{
    if (bLocalOdometry) { dsPic_data.dRPMtoMPS = value; }
    else
    {
 ...
 */
void summit_controller_dspic::ReadOdometry() { iCmd = DSPIC_CMD_READ_ODOMETRY; }
 ...
 */
void summit_controller_dspic::ReadEncoder() { iCmd = DSPIC_CMD_READ_ENCODER; }
...
/*!	\fn void summit_controller_dspic::SetMotorReferences(double rads, double
 * mps) \brief Sets motor references 1-position in rad, 2-speed in mps
 */
...
/*!	\fn void summit_controller_dspic::SetTrimmer(int channel, int offset1, int
 * center, int offset2) \brief Sets the trimmer parameters
 */
void summit_controller_dspic::SetTrimmer(
    int channel, int offset1, int center, int offset2)
{
    // Check channel
 ...
 */
float summit_controller_dspic::GetVoltage() { return (float)dsPic_data.dBatt; }
...
/*!	\fn void summit_controller_dspic::GetGyroValues(double *angle, double
 * *offset)
 *
 */
...
/*!	\fn void summit_controller_dspic::GetMotorReferences(double *rad, double
 * *mps) Returns the value of current references
 */
...
void summit_controller_dspic::UpdateOdometry()
{
    ...
    // Odometry calculation (using motor speed and position of the motor
    // direction)
    double fBetaRads =
        dsPic_data.rad;  // current orientation angle (for ackerman odometry)
    ...
    // This function can be called at higher freq than readings, so avoid
    // integrate twice
    dsPic_data.iEncoderAnt = dsPic_data.iEncoder;
    ...
    double d =
        dsPic_data
            .dDWheels;  // distance between center of back axis to front axis
    ...
    // Orientation
    if (amMode == ODOMETRY)
    {  // Takes robot angle from odometry
        dsPic_data.vel_yaw = (v_mps / d) * sin(fBetaRads);
        dsPic_data.odometry_yaw += dsPic_data.vel_yaw * fSamplePeriod;
    }
    else
    {  // Takes robot angle from gyro
        double pa =
            (dsPic_data.dGyro - dsPic_data.dOffsetGyro);  // 180.0 * DSPIC_PI;
...
/*!     \fn void summit_controller_dspic::cmdVelCallback(const
...
int main(int argc, char** argv)
{
    ...
    while (rclcpp::ok())
    {
        ...
        odom_trans.transform.translation.x =
            summit_controller->dsPic_data.odometry_x;
